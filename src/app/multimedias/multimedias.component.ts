...
export class MultimediasComponent {
        ...
        (await ancestryDatabase.multimedia.toArray())
          ...
          .forEach((next) => {
            ...
            if (
              xrefMatch != null &&
              nextMatch != null &&
              Number(xrefMatch[1]) < Number(nextMatch[1])
            ) {
              xref = next;
            }
          });
  ...
  submitForm() {
    ancestryDatabase
      .transaction("rw", [ancestryDatabase.multimedia], async () => {
        const gedcomMultimedia = new GedcomMultimedia(await this.nextXref());
        gedcomMultimedia.filePath =
          this.reactiveForm.controls.filePath.value.trim() || undefined;
        gedcomMultimedia.mediaType =
          this.reactiveForm.controls.mediaType.value.trim() || undefined;
